...
class ChartTimeRange
  ...
  constructor: (spec) ->
    ...
    * **workDayEndBefore** is an optional object in the form {hour: 17, minute: 0}. If minute is zero it can be omitted.
    ...
    if spec.step?
      @step = spec.step
    else if spec.endBefore? and @startOn?.greaterThan(@endBefore)
      @step = -1
    ...
    utils.assert(
      ...
      (spec.startOn? and spec.limit? and @step > 0) or
      (spec.endBefore? and spec.limit? and @step < 0),
